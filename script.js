document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateFileList() {
        ...
        const sizeText = (translations[currentLang] && translations[currentLang].size_info) || 'Size: {0} KB';
        ...
        selectedFiles.forEach((file, index) => {
            ...
            fileSize.textContent = sizeText.replace('{0}', fileSizeKB);
        ...
        return new Promise((resolve) => {
            ...
            worker.onmessage = function(e) {
                ...
                if (e.data.success) {
                    ...
                    // 处理需要回落到主线程的情况
                    console.log('Worker请求使用主线程处理图像');
                    worker.terminate();
                    
                    // 更新状态显示
                    const fallbackText = getTranslatedText('converting_fallback', 'Converting with fallback...');
                    ...
                    if (e.data.imageData) {
                        ...
                        processCanvasToBlob(canvas, file, e.data.quality, e.data.format, index, statusElement, resolve);
                    } else {
                        // 完全回落到fallbackConversion
                        fallbackConversion(file, index, resolve);
                    }
                } else {
...
// 辅助函数：将canvas转换为blob并处理结果
function processCanvasToBlob(canvas, file, quality, format, index, statusElement, resolve) {
...
function downloadImage() {
    if (selectedFiles.length === 1 && convertedBlobs.size === 1) {
        ...
        const newFilename = getOutputFilename(selectedFiles[0].name, selectedFormat);
...
downloadAllButton.addEventListener('click', () => {
    ...
    if (convertedBlobs.size === 1) {
        ...
        const filename = getOutputFilename(selectedFiles[0].name, selectedFormat);
    ...
    selectedFiles.forEach((file, index) => {
        if (convertedBlobs.has(file.name)) {
            ...
            const filename = getOutputFilename(file.name, selectedFormat);
